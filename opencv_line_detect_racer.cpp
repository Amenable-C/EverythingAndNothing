...
int main(int argc, char **argv)
{
   ...
   */
   ...
   float  steering_conversion = 0.14;
   
   
   namedWindow("Camera Image", WINDOW_NORMAL);
   resizeWindow("Camera Image", IMG_Width/4,IMG_Height/4);
     ...
     */
     ...
     line_count = line_center_x = 0.0;
	  ...
	  for(int i=0; i<linesP.size();i++)
	  {
		...
		int cy2 = linesP[i][3];
		*/
		c[i] = ((float)L[2]-(float)L[0])/((float)L[3]-(float)L[1]);
		d[i] = (float)L[0]-(float)c[i] *(float)L[1] ;
     ...
     if(line_count != 0) 
     {
        ...
        line_center_x = line_center_x / (float)line_count - img_width/2;
