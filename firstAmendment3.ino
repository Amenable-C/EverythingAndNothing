...
int Flex22 = A5;
int Flex23 = A6;
#define MAX 1023 // Flex Sensors 최댓값. 실험을 통해 정하기
#define MID 512 // Flex Sensors 중간값. 실험을 통해 정하기
...
void GameStart1(){
  ...
  Score1 = 0; 
  Score2 = 0; // 다른 게임을 할때 점수를 0,0으로 세팅해주기 위한 것.
}
...
void GameStart3(){
  StartTune3(); // 시작음 재생
  time_current = millis(); //
  outcome = TIE; // 전역변수라서 TIE로 세팅 // 안해도 상관없기는 함
  while(1){
    digitalWrite(trigPin, HIGH);
    delay(10);
    digitalWrite(trigPin, LOW);
    float duration = pulseIn(echoPin, HIGH); // 돌아오는 시간 계산
    float distance = duration * 340 / 10000 / 2; // 거리 계산
    ...
    if(state1 || state2 || state3 || state4 || state5 || state6){ // 하나라도 버튼이 눌러진 경우
        outcome = LOSE;
        Vibration(outcome); // 승패 결과를 진동으로 알려주기
        ScoreBoard(); // 점수판 수정
        
        outcome = TIE;
        state1 = false; state2 = false; state3 = false; state4 = false; state5 = false; state6 = false; // 혹시나 해서 다시 리셋
    ...
    whlie(Player1Hand < 20){ // 손이 다 펴졌을 때 // 멈춰야 하는 상황일 때
      ...
      Player1Hand = analogRead(Flex11); // 이렇게 다시 읽으면서, 다시 게임을 진행하는건지 아닌지 확인
    }
    if(Score1 > 2 || Score2 >2){ // 한명이 3점을 획득하여 게임을 종료해야 하는 상황
      EndTune(); // 게임 종료 음 재생
      FinalResult(Score1, Score2); // 최종 승자 정하기
      Score1 = 0; 
      Score2 = 0; // 다른 게임을 할때 점수를 0,0으로 세팅해주기 위한 것.
      break; // 가장 큰 while문을 나가면서 게임을 끝마치는 것.
    }
  }
}
